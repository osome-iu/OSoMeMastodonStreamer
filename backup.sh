...
dir_byte_sum() {
    ...
    # Use find to get sizes of all regular files, sum them
    # This avoids counting directory metadata overhead
...
if [ -d "$input_directory" ]; then
    ...
    for root in "${backup_roots[@]}"; do
        ...
        # Copy all content to the backup location using rsync (more robust)
        if rsync -a "$input_directory"/ "$target_directory"/; then
